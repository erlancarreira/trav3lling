...
class Upload {
    ...
    private $height = 2000; //800px
    private $width = 3000; //1546px
    private $size = 2500000; //1.5MB;
    ...
    private function setNewName($file = null) {
        if ($file != null) {
            ...
            $this->new_name_file = $name[0].'_trav3lling_'.uniqid(rand()). '.' . $ext['extension'];
        }

        // else {
        // 	print_r($this->file['name']);
        //     $ext = pathinfo($this->file['name']);
        //     $this->new_name_file = hash("sha256", uniqid(rand(), true)) . '.' . $ext['extension'];
        // }
    }
